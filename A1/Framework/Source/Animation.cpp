...
void Animation::Draw()
{
	...
	mat4 i(1.0f);//identitiy matrix
	glBindVertexArray(mVAO);
	glBindBuffer(GL_ARRAY_BUFFER, mVBO);
	GLuint WorldMatrixLocation = glGetUniformLocation(Renderer::GetShaderProgramID(), "WorldTransform");
	glUniformMatrix4fv(WorldMatrixLocation, 1, GL_FALSE, &i[0][0]);
...
//but the GetAnimationWorldMatrix will never call
glm::mat4 Animation::GetAnimationWorldMatrix() const
{
	...
	int firstFrame;
	int secondFrame;
	int div = mKeyTime[1] - mKeyTime[0];
		float x = mCurrentTime;
		while (x > mKeyTime[mKeyTime.size() - 1]) {
			x = mCurrentTime - mKeyTime[mKeyTime.size() - 1];
		}
		...
		//glm::quat q1 = glm::angleAxis(mKey[firstFrame].GetRotationAngle(), mKey[firstFrame].GetRotationAxis());
		//glm::quat q2 = glm::angleAxis(mKey[secondFrame].GetRotationAngle(), mKey[secondFrame].GetRotationAxis());
	   
		worldMatrix = glm::translate(worldMatrix, mix(mKey[firstFrame].GetPosition(), mKey[secondFrame].GetPosition(), (mCurrentTime- mKeyTime[firstFrame])/ div));
		//worldMatrix = glm::rotate(worldMatrix, mKey[0].GetRotationAngle(), mKey[0].GetRotationAxis());
		//worldMatrix = glm::rotate(worldMatrix,mKey[1].GetRotationAngle(), mKey[1].GetRotationAxis());
		//worldMatrix = worldMatrix * glm::toMat4(slerp(q1, q2, (mCurrentTime - mKeyTime[firstFrame]) / div));
		
		worldMatrix = glm::rotate(worldMatrix, mix(glm::radians(mKey[firstFrame].GetRotationAngle()), glm::radians(mKey[secondFrame].GetRotationAngle()), (mCurrentTime - mKeyTime[firstFrame]) / div), mix(mKey[firstFrame].GetRotationAxis(), mKey[secondFrame].GetRotationAxis(), (mCurrentTime - mKeyTime[firstFrame]) / div));
		worldMatrix = glm::scale(worldMatrix, mix(mKey[firstFrame].GetScaling(), mKey[secondFrame].GetScaling(), (mCurrentTime - mKeyTime[firstFrame]) / div));
